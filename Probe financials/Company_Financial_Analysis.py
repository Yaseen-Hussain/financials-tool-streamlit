...
folder_path = r"C:\Users\YaseenHussain\Documents\Solar\PPT\Tenders PPT"

# Fields to look up from Standalone Financial Data
financial_fields = [
    "Net Revenue",
    "Cost of Materials Consumed",
    "Gross Profit Margin (%)", #Depends
    "EBITDA Margin (%)",
...
for file in os.listdir(folder_path):
    if file.endswith(".xls"):
        ...
        try:
            wb_about = xlrd.open_workbook(file_path)
            ...
            # Determine the "latest" column (first non-empty cell in row 0)
            latest_col = None
            date_of_report = None
            ...
            # Create dictionary for all lookup fields
            fin_data = {}
            ...
            # Now calculate all derived fields
            ...
            # Calculated fields
            #  gross_margin = (net_revenue - fin_data.get("Cost of Materials Consumed",0))/net_revenue
            ...
            output_data.append({
                ...
                "EBITDA (%)": fin_data.get("EBITDA Margin (%)")/100 if fin_data.get("EBITDA Margin (%)") is not None else None,
                ...
                "ROCE (%)": fin_data.get("Return on Capital Employed (%)")/100 if fin_data.get("Return on Capital Employed (%)") is not None else None,
                "ROE (%)": fin_data.get("Return on Equity (%)")/100 if fin_data.get("Return on Equity (%)") is not None else None,
...
#df[fields_to_round] = df[fields_to_round].round(3)
df = df.sort_values(by="Net Revenue", ascending=False)
output_file = os.path.join(folder_path, "financial_summary_output.xlsx")    
df.to_excel(output_file, index=False)
print("Done. Output written to financial_summary_output.xlsx")
